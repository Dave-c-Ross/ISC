...
const String syncUrl = "http://192.168.2.46:32080/david.cuerrier/irrigation-system/-/raw/master/home.html";

WebServer server(80);
HTTPClient syncHttp;
ElasticsearchClient esc("http://192.168.2.46:9200", "sprinkler_log");
WiFiUDP udp;
...
void stopZone1() {
  writeLog("TIMER", "From timer turn OFF zone 1");
  toogleSp(SP_1_PIN, false);
}

void stopZone2() {
  writeLog("TIMER", "From timer turn OFF zone 2");
  toogleSp(SP_2_PIN, false);
}
...
void writeMetric(String feature, String metric, int zone, float value) {
  esc.sendMetric(String(now(), DEC), feature, metric, zone, value, "sprinkler_metric");
}
void writeMetric(String feature, String metric, int zone, String value) {
  esc.sendMetric(String(now(), DEC), feature, metric, zone, value, "sprinkler_metric");
}
...
void toogleSp(int pin, bool state) {
  ...
  if (state) {
    writeLog("TOOGLE", String("Asking to turn ON zone " + String(pin) + "..."));
  } else {
    writeLog("TOOGLE", String("Asking to turn OFF zone " + String(pin) + "..."));
    writeMetric("TOOGLE", "state", zone, 1);
  }

  // Turn on reverser if needed
  if (!state) {
    writeLog("TOOGLE", "Reverse polarity to prepare turn off.");
    digitalWrite(REVERSE_POLARITY_PIN, !state);
    delay(500);
  }

  writeLog("TOOGLE", "Priming the solenoid...");
  // Prime the solenoid
  digitalWrite(pin, HIGH);
  delay(1000);
  digitalWrite(pin, LOW);
  writeLog("TOOGLE", "Priming the solenoid... DONE");

  for (int i = 0; i <= 1; i++) {
    // Charge the booster !!
    writeLog("TOOGLE", "Charging booster time " + String(i));
    digitalWrite(BOOSTER_PIN, HIGH);
    delay(3000);
    digitalWrite(BOOSTER_PIN, LOW);
    writeLog("TOOGLE", "Charging booster time " + String(i) + " DONE");

    // Strike the solenoid and wait 1.5 sec
    writeLog("TOOGLE", "Striking solenoid time " + String(i));
    digitalWrite(pin, HIGH);
    delay(1500);
    // Stop striking the solenoid witch will be left in its position
    writeLog("TOOGLE", "STOP Striking solenoid time " + String(i));
    digitalWrite(pin, LOW);
  }

  delay(500); // Wait a bit befor turning back the polarity to avoid backfire in the solenoid
  writeLog("TOOGLE", "Stop reversing polarity");
  digitalWrite(REVERSE_POLARITY_PIN, LOW);
}
